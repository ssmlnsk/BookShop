...
class MainWindow(QMainWindow):
    ...
    def new_book(self):
        ...
        if title_book != '' and cost_book != '' and genre != '' and author != '' and ph != '':
            ...
            self.mes_box('Ошибка', 'Заполните все поля!')
    ...
    def add_new_request(self):
        ...
        if client != '' and book != '':
            book_name = self.comboBox_book.currentText()
            book_id = self.facade.get_book_id(book_name)
            count_book = self.facade.get_count_book(book_id)
            if count_book == 0:
                self.mes_box('Ошибка', 'Книги нет в наличии!')
            else:
    ...
    def add_book_to_request(self):
        ...
        if self.block_req is True:
            ...
            self.mes_box('Ошибка', 'Заказ уже сформирован!')

    def save_request(self):
        """
        Сохранение заказа в базу данных.
        """
        if self.ui.add_new_field.count() != 0:
            if self.block_req is False:
                ...
                self.mes_box('Ошибка', 'Создайте штрих-код!')
        else:
            self.mes_box('Ошибка', 'Сформируйте заказ!')
    ...
    def generateCode(self):
        ...
        """
        if self.ui.add_new_field.count() == 0:
            self.mes_box('Ошибка', "Заказ не сформирован!")
        elif self.block_req != False:
            rv = BytesIO()
            ...
            self.mes_box('Ошибка', 'Штрих-код уже создан')
    ...
    def add_book_to_pr_od(self):
        ...
        if self.listWidgetDelivery.count() != 0:
            ...
            if quantity >= 1:
                ...
                self.mes_box('Ошибка', 'Невозможно добавить книгу! Заявка не создана!')
        else:
            self.mes_box('Ошибка', 'Заявка не создана!')
...
class DialogAuth(QDialog):
    ...
    def enter(self):
        ...
        if auth_log == '' or auth_pas == '':
            ...
            self.mes_box('Ошибка', 'Введите капчу!')
        else:
            ...
            if self.now_captcha is not None and self.now_captcha != self.ui.edit_captcha.text():
                ...
                self.mes_box('Ошибка', 'Неправильно введена капча.')
                ...
                if self.count_try_entry == 2:
                    ...
                    self.mes_box('Ошибка', 'Вторая неуспешная попытка входа. Теперь введите капчу.')
                else:
                    logging.log(logging.INFO, 'Ошибка. Неправильно введены данные.')
                    self.mes_box('Ошибка', 'Неправильно введены данные.')
                ...
                if self.count_try_entry >= 3 and self.next_try > now_time:
                    ...
                    self.mes_box('Ошибка', 'Подождите, прежде чем пытаться вводить снова.')
                    return
